...
out.dir <- '~/yuzhao1/work/manu/rc2/plots/1rna_annotation_UnionHeatmap/'
```
...
labels <- rna.unionHeatmap.markers %>% unlist()
...
col_fun = colorRamp2(seq(-2, 10, by = 12/10), rc2_rna_heatmap_colors_gradient1[2:12])
# col_fun = colorRamp2(seq(-2, 8, by = 10/13), c(rep('white', 4), manual_colors_gradient3))

for (virdis_option in LETTERS[1:8]){
  pdf(paste0(out.dir, 'virdis_',virdis_option, '_union.pdf'), width = 8, height = 8)
p1 <- Heatmap(Average.expression.mtx, 
              column_split = factor(c(rep('epithelial', length(rna.epithelial.markers)),
                                    rep('tcell', length(rna.tcell.markers)),
                                    rep('bcell', length(rna.bcell.markers)),
                                    rep('myeloid', length(rna.myeloid.markers)),
                                    rep('others', length(rna.others.markers))),
