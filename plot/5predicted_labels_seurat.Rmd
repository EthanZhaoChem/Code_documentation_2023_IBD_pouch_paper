...
out.dir <- '/project/gca/yuzhao1/work/manu/rc2/plots/5seurat_integration/predicted_labels/'
```
...
p <- plot_df_umap_custom(df, show.label = 'name', custom_colors = custom_colors_rna_all)

pdf(paste0(out.dir, 'union.pdf'), width = 8, height = 10, pointsize = 1)
p
dev.off()
...
pdf(paste0(out.dir, 'union_NoName.pdf'), width = 8, height = 10, pointsize = 1)
p
dev.off()
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_immune))]
```
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_epithelial))]
```
...
p<-plot_df_umap_custom(df, show.label = 'na', custom_colors = custom_colors_rna_epithelial)
p
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_tcell))]
```
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_bcell))]
```
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_myeloid))]
```
...
```{r}
unique(proj$predictedGroup_lineage_constrained)[which(!unique(proj$predictedGroup_lineage_constrained) %in% names(custom_colors_rna_others))]
```
...
p<-plot_df_umap_custom(df, show.label = 'na', custom_colors = custom_colors_rna_others)
p
