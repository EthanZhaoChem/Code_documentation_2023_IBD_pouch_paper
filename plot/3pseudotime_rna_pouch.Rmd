...
```{r}
seurat <- subset(epithelial, biopsy_location == 'POU')
```
...
pdf(paste0(dir.out, 'pouch_EC1.pdf'), width = 7, height = 25, pointsize = 1)
p <- Heatmap(
   df2,
...
# sorts the rows of df2 based on the index of the maximum value in each row
df2 <- df2[order(apply(df2, 1, which.max)), ]
pdf(paste0(dir.out, 'pouch_EC1_tf.pdf'), width = 7, height =10, pointsize = 1)
p <- Heatmap(
   df2,
   # col = rc2_rna_heatmap_colors_gradient1, 
   cluster_rows=F, cluster_columns=FALSE,
...
df2 <- df2[order(apply(df2, 1, which.max)), ]
pdf(paste0(dir.out, 'pouch_EC2.pdf'), width = 7, height = 19, pointsize = 1)
p <- Heatmap(
   df2,
   # col = rc2_rna_heatmap_colors_gradient1, 
   cluster_rows=F, cluster_columns=FALSE,
...
# sorts the rows of df2 based on the index of the maximum value in each row
df2 <- df2[order(apply(df2, 1, which.max)), ]
pdf(paste0(dir.out, 'pouch_EC2_tf.pdf'), width = 7, height =10, pointsize = 1)
p <- Heatmap(
   df2,
   # col = rc2_rna_heatmap_colors_gradient1, 
   cluster_rows=F, cluster_columns=FALSE,
