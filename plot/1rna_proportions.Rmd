...
```{r}
out.dir <- '~/yuzhao1/work/manu/rc2/plots/1rna_proportion/epithelial/'
```

```{r}
seurat <- subset(epithelial, cells = Cells(epithelial)[which(epithelial$anno1 %in% c("EC1-1", "EC1-2", "EC2-1", "EC2-2"))])
seurat$anno1.loc <- seurat$anno1_loc
seurat$anno1.loc[seurat$anno1 %in% c("EC1-1", "EC1-2") & seurat$biopsy_location =='POU'] <- 'EC-POU'
seurat$anno1.loc[seurat$anno1 %in% c("EC2-1", "EC2-2") & seurat$biopsy_location =='POU'] <- 'EC-POU'
...
```{r}
xx.EC <- table(seurat@meta.data[, c('anno1', 'anno1.loc')]) %>% as.data.frame.matrix() 
...
p1 <- Heatmap(xx.EC, col = manual_colors_gradient3, cluster_columns = F, cluster_rows = F, name = 'cell percentage', show_row_names = F,
        rect_gp = gpar(col = "white", lwd = 2), cluster_row_slices = FALSE,
        border = T, column_gap = unit(3, "mm"),
        row_split = factor(row.split, levels = c('EC-AC', 'EC-POU2', 'EC-POU1', 'EC-PP', 'EC-TI')),
        use_raster = F
        )
...
```{r}
seurat <- subset(epithelial, cells = Cells(epithelial)[which(epithelial$anno1 %in% c("Stem"))])
seurat$anno1.loc <- seurat$anno1_loc
seurat$anno1.loc[seurat$anno1 %in% c("Stem") & seurat$biopsy_location =='POU'] <- 'Stem-POU'

seurat$anno1 <- seurat$anno2
```

```{r}
xx.Stem <- table(seurat@meta.data[, c('anno1', 'anno1.loc')]) %>% as.data.frame.matrix() 
...
# epithelial Stem, pou1 pou2, pp1 pp2
...
```{r}
seurat <- subset(epithelial, cells = Cells(epithelial)[which(epithelial$anno1 %in% c("Goblet1", "Goblet2"))])
...
```{r}
xx.Goblet <- table(seurat@meta.data[, c('anno1', 'anno1.loc')]) %>% as.data.frame.matrix() 
xx.Goblet <- scale(xx.Goblet, center = F, scale= xx.Goblet%>% colSums(.))
xx.Goblet <- t(xx.Goblet)
```
...
```{r}
seurat <- union
out.dir <- '~/yuzhao1/work/manu/rc2/plots/1rna_proportion/union/'
```

```{r}
# per location
df <- data.frame((seurat@meta.data))[,c('lineage', 'biopsy_location')]
...
```{r}
# per sample
seurat$sample <- paste0(seurat$Patient_ID,'-',seurat$biopsy_location)
df <- seurat@meta.data[,c('lineage', 'sample')]
df$xx1 <- rep(1, nrow(df))
df$xx1.sum <- 0
df2 <- aggregate(xx1~.,df,FUN=sum)
...
p2 <-ggplot(df2, aes(x = sample, y = xx1, fill = lineage)) +
  geom_bar(position = "stack", stat = "identity")+
  scale_fill_manual(values = custom_colors_rna_lineage)+
  labs(title = "", y = "Proportion of cells (%)", x = "", fill = "")+
  theme.composition.sample

pdf(paste0(out.dir, 'sample_number.pdf'), width = 9, height = 6, pointsize = 1)
print(p2)
dev.off()
```
...
for (lineage.i in unique(union$lineage)){
  ...
  color.values = eval(as.name(paste0('custom_colors_rna_', lineage.i)))
  out.dir <- paste0('~/yuzhao1/work/manu/rc2/plots/1rna_proportion/', lineage.i, '/')
  
  
  ### plot location.pdf
  df <- seurat@meta.data[,c('anno1', 'biopsy_location')]
  df$xx1 <- rep(1, nrow(df))
  df$xx1.sum <- 0
  df2 <- aggregate(xx1~.,df,FUN=sum)
  ...
  ### plot sample.pdf
  seurat$sample <- paste0(seurat$Patient_ID,'-',seurat$biopsy_location)
  df <- seurat@meta.data[,c('anno1', 'sample')]
...
# epithelial anno2
seurat <- eval(as.name(lineage.i))
color.values = custom_colors_rna_epithelial_anno2

out.dir <- paste0('~/yuzhao1/work/manu/rc2/plots/1rna_proportion/', lineage.i, '/')


### plot location.pdf
df <- seurat@meta.data[,c('anno2', 'biopsy_location')]
df$xx1 <- rep(1, nrow(df))
df$xx1.sum <- 0
df2 <- aggregate(xx1~.,df,FUN=sum)
...
### plot sample.pdf
seurat$sample <- paste0(seurat$Patient_ID,'-',seurat$biopsy_location)
df <- seurat@meta.data[,c('anno2', 'sample')]
...
# plot
pdf(paste0(out.dir, 'sample_anno2.pdf'), width = 9, height = 6, pointsize = 1)
print(p2)
dev.off()
```
