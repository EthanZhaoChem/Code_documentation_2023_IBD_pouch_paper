LoadXenium_alt <- function(data.dir, fov = "fov", assay = "Xenium") 
{
  data <- ReadXenium_alt(data.dir = data.dir, type = c("centroids", 
                                                       "segmentations"), )
...
ReadXenium_alt <- function(data.dir, outs = c("matrix", "microns"), type = "centroids", 
                           mols.qv.threshold = 20) 
{
  ...
  data <- sapply(outs, function(otype) {
    switch(EXPR = otype, matrix = {
      ...
      if (has_dt) {
        tx_dt <- arrow::read_parquet(file.path(data.dir, "transcripts.parquet"))
