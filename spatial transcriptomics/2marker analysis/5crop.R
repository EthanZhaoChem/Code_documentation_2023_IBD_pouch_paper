...
# set path
out.dir <- '~/yuzhao1/work/final_RC2rna/0revision/spatial2/plots/cropped/'

##################  ################ ##################  ################
# pre select a few genes to plot
genes_selected <- c("FABP6", 'APOA4', "CEACAM5", 'CD24', "CA2")
max_cutoffs <- c(3,3,3, 3, 3)
...
p3 <- ImageDimPlot(xenium.obj, fov = "zoom", cols = custom_colors_rna_all, group.by = 'predicted.celltype', coord.fixed = T)
...
pdf(paste0(out.dir, tmp.sample, '_part3.pdf'), height = 8, width = 10)
print(p3)
dev.off()
