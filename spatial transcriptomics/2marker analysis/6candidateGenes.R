...
##################  ################ ##################  ################
# set path
out.dir <- '~/yuzhao1/work/final_RC2rna/0revision/spatial2/plots/candidate_tfs3/'


# pre select a few genes to plot
# genes_selected <- c('CDX1', 'FOXP1', 'KLF5', 'EHF',  'NFIA', 'BACH1')
# genes_selected <- c('ESRRG', 'MAF','TBX3', 'HNF4G',  'NR1H4', 'PPARA')
genes_selected <- c('GATA4', 'GATA5')
# genes_selected <- c('CEACAM5', 'CD24','CA2', 'FABP6','APOA4')
max_cutoffs <- rep(3, 2)
min_cutoffs <- rep(0, 2)
p2.height <- 16
p2.width <- 10
...
p1 <- ImageDimPlot(xenium.obj, fov = "zoom", axes = F, border.color = "white",
                   border.size = 0.1, cols = "polychrome",coord.fixed = T,
...
p2 <- ImageFeaturePlot(xenium.obj,fov = 'zoom',
                       ...
                       size = 0.75, 
                       cols = c("white", "red"))+ 
  plot_layout(nrow = p2.nrow)

pdf(paste0(out.dir, tmp.sample, '_part1.pdf'), height = 5, width = 6)
print(p1)
dev.off()

pdf(paste0(out.dir, tmp.sample, '_part2.pdf'), height = p2.height, width = p2.width)
print(p2)
dev.off()
