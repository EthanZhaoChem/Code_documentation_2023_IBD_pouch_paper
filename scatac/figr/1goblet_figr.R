...
proj <- loadArchRProject(path = "~/yuzhao1/work/final_RC2atac/dataset/gut_2kmin_6TSS_DoubletRatio2_epithelial_filtered2_Goblet/")
out.dir <- '~/yuzhao1/work/final_RC2atac/figr/goblet/'
########################## :) I am a divider :) ##########################
# #  get gene expression
# #  read files
# addArchRThreads(1)
...
#                            n_bg = 100)
# saveRDS(cisCorr, paste0(out.dir, 'cisCorr.rds'))

 
# ########################## :) I am a divider :) ##########################

# 2. Filter peak-gene correlations by p-value
cisCor <- readRDS(paste0(out.dir, 'cisCorr.rds'))
...
# dorcGenes <- names(table(cisCor.filt$Gene))[table(cisCor.filt$Gene) >= 7]
# saveRDS(dorcGenes, paste0(out.dir, 'dorcGenes.rds'))
...
# saveRDS(intersect(dorcGenes, human_tfs), '~/yuzhao1/work/final_RC2atac/figr/goblet/dorcTFs.rds')
...
# names(ATAC.se.paired@assays) <- 'counts'
# 
# dorcMat <- getDORCScores(ATAC.se.paired, dorcTab=cisCor.filt, geneList=dorcGenes,nCores=18)
# saveRDS(dorcMat, paste0(out.dir, 'dorcMat.rds'))
...
figR.d <- runFigRGRN(ATAC.se = ATAC.se.paired, # Must be the same input as used in runGenePeakcorr()
                     ...
                     rnaMat = RNAmat.paired,
                     nCores = 8)
saveRDS(figR.d, paste0(out.dir, 'figR.d.rds'))
...
# library(BuenColors)
# png(paste0(out.dir, '/DORC-to-TF associations.png'),res = 300, height = 1000, width = 1200)
# figR.d %>%
...
# pdf(paste0(out.dir, 'figRHeatmap.pdf'), width = 10, height = 26)
...
# pdf(paste0(out.dir, 'figRHeatmap_intersectDORC_ECdegRNA.pdf'), width = 10, height = 20)
...
# 
# library(networkD3)
# png(paste0(out.dir, 'figR_network.png'),res = 300, height = 3000, width = 3000)
