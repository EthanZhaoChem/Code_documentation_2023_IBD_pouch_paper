...
# read files
proj <- loadArchRProject(path = "~/yuzhao1/work/final_RC2atac/dataset/gut_2kmin_6TSS_DoubletRatio2_tcell/")
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_un.png',width = 2600, height = 3000,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
...
gene <- c('IGHA2', 'IGHA1', 'CD27', 'SDC1', 'SLAMF7', 'CD3D', 'XBP1', 'PTPRC', 'LGR5', 'EPCAM',
          ...
          gca.mesenchymal.markers %>% unlist())  %>% unique()%>% intersect(., proj@geneAnnotation$genes$symbol)
...
idxPass <- which(!proj$Harmony_Clusters_res1.5 %in% c('C1', 'C2'))
cellsPass <- proj$cellNames[idxPass]
proj.filtered <- subsetArchRProject(
  ArchRProj = proj,
  cells = cellsPass,
  outputDirectory = "~/yuzhao1/work/final_RC2atac/dataset/gut_2kmin_6TSS_DoubletRatio2_tcell_filtered",
  dropCells = TRUE,
  logFile = NULL,
