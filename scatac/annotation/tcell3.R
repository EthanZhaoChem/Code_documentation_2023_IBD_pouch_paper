...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_anno1.png',width = 2600, height = 3000,res = 300)
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_anno1_loc.png',width = 5000, height = 5200,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
df$cluster_name <- proj$anno1
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_res1.5.png',width = 2600, height = 3000,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_res2.5.png',width = 2600, height = 3000,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_res5.png',width = 2600, height = 3000,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
...
rna_ref <- readRDS('~/yuzhao1/work/final_RC2rna/annotation/rds/tcell2.rds')
...
proj<- addGeneIntegrationMatrix(
  ...
  sampleCellsATAC = 10444,
  sampleCellsRNA = 16389,
  dimsToUse = 1:50,
...
png('~/yuzhao1/work/final_RC2atac/annotation/plots/tcell/umap_Co_lineage.png',width = 2600, height = 3000,res = 300)
df <- data.frame(proj@cellColData)
df$embedding1 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_1`
df$embedding2 <- proj@embeddings$Harmony_UMAP$df$`Harmony#UMAP_Dimension_2`
...
gene2 <- c('KLRF1', 'IFNG', 'FGFBP2', 'GZMB', 'CX3CR1', 'KLRG1', # NKT, has CD3D
           ...
           'SLC4A10', 'NCR3', 'KLRG1', 'GZMK', 'KLRG1') %>% unique() %>% intersect(., proj@geneAnnotation$genes$symbol) # MAIT, has CD3
...
plotPDF(plotList = p2,
        name = paste0(c('Violin_tcell', ".pdf")),
        ArchRProj = proj,
        addDOC = FALSE, width = 5, height = 5)

p3 <- plotGroups(
  ArchRProj = proj,
  groupBy = "predictedGroup_Co_lineage",
  colorBy = "GeneScoreMatrix",
  name = gene2,
  plotAs = "violin",
  alpha = 0.4,
...
plotPDF(plotList = p3,
        name = paste0(c('Violin_tcell_predictedCell_Co_lineage', ".pdf")),
        ArchRProj = proj,
        addDOC = FALSE, width = 5, height = 5)
...
# idxPass <- which(!proj$Harmony_Clusters_res1.5 %in% c('C7'))
# cellsPass <- proj$cellNames[idxPass]
# proj.filtered <- subsetArchRProject(
#   ArchRProj = proj,
#   cells = cellsPass,
#   outputDirectory = "~/yuzhao1/work/final_RC2atac/dataset/gut_2kmin_6TSS_DoubletRatio2_tcell_filtered2",
#   dropCells = TRUE,
#   logFile = NULL,
