...
# 2. metadata, POU_EC2vsPOU_EC1
metadata.original$anno1[metadata.original$anno1 %in% c("EC1-1", "EC1-2", "EC2-1", "EC2-2")] <- 'EC'
...
metadata.original <- metadata.original[metadata.original$anno1.sub %in% c("AC_EC", "TI_EC"),]
...
# 6. differential test: should be at least 3 pesudo bulks
...
results <- modelfit
saveRDS(results ,'~/yuzhao1/work/final_RC2atac/dar_great/rds/da_results_AC_ECvsTI_EC.rds')
