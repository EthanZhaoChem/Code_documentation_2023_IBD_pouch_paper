...
```{r}
seurat <- subset(seurat_all, cells = Cells(seurat_all)[seurat_all$anno1 %in% c('Goblet1', 'Goblet2')])
...
seurat <- subset(seurat, cells = Cells(seurat)[seurat$anno1%in%c('Goblet-AC', 'Goblet-POU2')])
...
Average.expression.mtx <-
  Average.expression.mtx[ rownames(df), c(paste0('Goblet-AC', '-', patients_control),
                                           paste0('Goblet-POU2', '-', patients_uc))]
...
```{r}
pdf(paste0(out.dir, 'AA_Goblet_sample_heatmap', plot_suffix, '.pdf'), width = 7, height = 7.5)
