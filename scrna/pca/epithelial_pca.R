...
seurat <- readRDS('~/yuzhao1/work/final_RC2rna/annotation/rds/epithelial3.rds')
...
FilePath.Sample_ID.mtx <- paste0(out.dir, 'epithelial_gene_vs_Sample_ID_RNA_count_mtx.rds') 
FilePath.pdf1 <- paste0(out.dir, 'epithelial_AllGenes_PC.pdf') 
FilePath.pdf2 <- paste0(out.dir, 'epithelial_AllGenes_PC1_ByLocation.pdf') 
FilePath.pdf3 <- paste0(out.dir, 'epithelial_AllGenes_PC2_ByLocation.pdf') 
...
ggplot(df, aes(x = PC1, y = PC2, label = Sample_ID)) +
  ...
  labs(x = "PC 1 (28.9% explained variance)", y = "PC 2 (12.7% explained variance)", title = "")+
  guides(fill = guide_legend(override.aes = list(shape = c(21))))
dev.off()


df$biopsy_location <- factor(df$biopsy_location, levels = c('AC','POU','PP','TI'))
my_comparisons <- list(c('AC','POU'), c('POU', 'TI'), c('PP', 'TI'))
...
ggboxplot(df, x = "biopsy_location", y = "PC1", fill = "biopsy_location", outlier.shape = NA, 
  ...
  labs(x = "Biopsy location", y = "PC 1 (28.9% explained variance)", title = "")
dev.off()
