...
FilePath.Sample_ID.mtx <- paste0(out.dir, 'stromal_gene_vs_Sample_ID_RNA_count_mtx.rds') 
FilePath.pdf1 <- paste0(out.dir, 'stromal_AllGenes_PC.pdf') 
FilePath.pdf2 <- paste0(out.dir, 'stromal_AllGenes_PC1_ByLocation.pdf') 
FilePath.pdf3 <- paste0(out.dir, 'stromal_AllGenes_PC2_ByLocation.pdf') 
...
ggplot(df, aes(x = PC1, y = PC2, label = Sample_ID)) +
  ...
  labs(x = "PC 1 (10.1% explained variance)", y = "PC 2 (8.0% explained variance)", title = "")+
  guides(fill = guide_legend(override.aes = list(shape = c(21))))
dev.off()


df$biopsy_location <- factor(df$biopsy_location, levels = c('AC','POU','PP','TI'))
my_comparisons <- list(c('AC','POU'), c('POU', 'PP'), c('PP', 'TI'), c('AC', 'TI'))
compare_means(PC1 ~ biopsy_location,  data = df,
...
ggboxplot(df, x = "biopsy_location", y = "PC1", fill = "biopsy_location", outlier.shape = NA, 
  ...
  labs(x = "Biopsy location", y = "PC 1 (10.1% explained variance)", title = "")
dev.off()


my_comparisons <- list(c('AC','POU'), c('POU', 'PP'), c('PP', 'TI'))
pdf(paste0(FilePath.pdf3), width = 5, height = 5)
ggboxplot(df, x = "biopsy_location", y = "PC2", fill = "biopsy_location", outlier.shape = NA, 
          bxp.errorbar=T, bxp.errorbar.width = 0.2)+ 
  geom_jitter(color="black", size=1.2, alpha=0.9, width = 0.1) +
  scale_fill_manual(values=manual_colors_rc2_location)+
  stat_compare_means(PC1 ~ biopsy_location, 
  ...
  labs(x = "Biopsy location", y = "PC 2 (8% explained variance)", title = "")
dev.off()
