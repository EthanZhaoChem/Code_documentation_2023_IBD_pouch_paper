...
FilePath.Sample_ID.mtx <- paste0(out.dir, 'union_gene_vs_Sample_ID_RNA_count_mtx.rds') 
FilePath.pdf1 <- paste0(out.dir, 'union_AllGenes_PC.pdf') 
FilePath.pdf2 <- paste0(out.dir, 'union_AllGenes_PC1_ByLocation.pdf') 
FilePath.pdf3 <- paste0(out.dir, 'union_AllGenes_PC2_ByLocation.pdf') 
...
ggplot(df, aes(x = PC1, y = PC2, label = Sample_ID)) +
  ...
  labs(x = "PC 1 (25.5% explained variance)", y = "PC 2 (20.1% explained variance)", title = "")+
...
ggboxplot(df, x = "biopsy_location", y = "PC1", fill = "biopsy_location", outlier.shape = NA, 
  ...
  labs(x = "Biopsy location", y = "PC 1 (25.5% explained variance)", title = "")
dev.off()
