...
```{r}
groupToTest <- 'CD8T'
seurat <- subset(tcell, cells = Cells(tcell)[which(tcell$anno1%in%c("CD103+ CD8 Trm", "KLRG1+ CD8 Trm"))])
seurat$anno1 <- 'CD8T'

group.ti <- paste0(groupToTest, '-TI')
group.pp <- paste0(groupToTest, '-PP')
group.pou <- paste0(groupToTest, '-POU')
group.ac <- paste0(groupToTest, '-AC')
```

```{r}
out.dir <- '~/yuzhao1/work/final_RC2rna/deg/tcell/CD8T/'
```
